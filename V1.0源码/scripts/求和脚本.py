...
class SumScript(BaseScript):
    name = '求和脚本'
    description = '对框选区域按行/列求和，输出一行或一列'
    ...
    def run(self, sheet, params):
        ...
        def _sum_text(vals: list[float], decs: list[int]) -> str:
            """求和 + 精度（组内最大位数 + 2，除尽不补零）。"""
            s = sum(vals)
            n = max(decs) + 2
            return _fmt(round(round(s, n), 10))
        ...
        if by_row:
            for r in range(r1, r2 + 1):
                ...
                results.append((r, _sum_text(vals, decs)))
        else:
            for c in range(c1, c2 + 1):
                ...
                results.append((c, _sum_text(vals, decs)))
