...
class BlockItem(QGraphicsObject):
    ...
    def _relayout_propagate(self):
        ...
        self._relayout()
        cur = self.parentItem()
        while isinstance(cur, BlockItem):
            cur._relayout()
            cur = cur.parentItem()
...
class BlockScene(QGraphicsScene):
    ...
    def _on_drag_finished(self, item: BlockItem, scene_pos: QPointF):
        ...
        iface = self.interface_at(scene_pos, item)
        if iface is not None and iface is not self._find_own_interface(item):
...
def _node_to_dict(node) -> dict:
    ...
    return {
        ...
        'output_target': node.output_target.value if node.output_target else None,
