"""除法脚本 — 第一个计算元为被除数，其余依次相除，输出一列或一行结果。

计算元：点选列/行、手动输入常数、从剪贴板接入（可添加多个）。
槽位顺序即运算顺序：第 1 个计算元 ÷ 第 2 个 ÷ 第 3 个 ÷ ...（常数同样参与顺序）。
识别规则：纯数字有效；空格子报错拒绝；含文字格识别为标题（占比 >30% 拒绝）。
保留小数位数（步骤 2 面板可选手动或默认）：
- 默认（自动）：逐位置取各计算元在该位置小数位数的最大值，**再加 2 位**
  （除法结果常产生无限小数，如 1÷3=0.333...；N = max(位数)+2）；
  若在 N 位内除尽则显示实际结果，不强行补 0（1.5÷0.25=6 显示 6）。
- 手动：全局固定 0-10 位；整数结果不补零（3 显示 3）。
除 0 策略：执行前先检查所有除数（含常数 0），任一为 0 立即报错中止，
不写入任何结果。
输出规则：标题格永远不写结果 ——
...
class DivideScript(BaseScript):
    name = '除法脚本'
    description = '第一个计算元除其余（列/行/常数/剪贴板），输出一列或一行'
    ...
    def steps(self):
        return [
            ...
            OperandInputStep('输入计算元：每个框点右侧箭头选择 列/行/常数/剪贴板（第 1 个为被除数）',
                             decimals=True, operator='÷'),
    ...
    def run(self, sheet, params):
        ...
        # 第 1 个计算元作为被除数（常数广播），其余依次作除数
        first = slots[0]
        if first['kind'] == 'constant':
            result = [first['value']] * data_len
        else:
            result = list(first['values'])

        # 除 0 检查：先检查后执行，任一除数为 0 立即中止，不写入任何结果
        # 数据位置单位：按行运算时数据在第 i+1 列；按列运算时数据在第 i+1 行
        unit = '列' if '行' in direction else '行'
        for s in slots[1:]:
            if s['kind'] == 'constant':
                if s['value'] == 0:
                    return f'❌ 除数为 0：计算元「{s["display"]}」为 0，已中止'
            else:
                for i, v in enumerate(s['values']):
                    if v == 0:
                        return (f'❌ 除数为 0：第 {i + 1} {unit}'
                                f'（{s["display"]}）为 0，已中止')
        ...
        if mode == 'manual':
            ...
            # 自动：逐位置取各计算元（常数取自身唯一位数）小数位数的最大值，再 +2 位
            rounded = []
            for i in range(data_len):
                n = 0
                for s in slots:
        ...
        def _fmt(v: float) -> str:
            """整数显示为 3/-3，小数显示为 3.5/-3.5（不出现 3.0）。"""
            if v == int(v):
                return str(int(v))
            return str(v)
