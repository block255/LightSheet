...
标题行/列位置不写入（保留原内容）。
极差：max(vals) - min(vals)（一组数据的最大值减最小值）。
...
class RangeScript(BaseScript):
    name = '极差脚本'
    description = '对框选区域按行/列求极差（最大值-最小值），输出一行或一列'
    ...
    def run(self, sheet, params):
        ...
        def _range_text(vals: list[float], decs: list[int]) -> str:
            """求极差 max-min + 精度（组内最大位数 + 2，除尽不补零）。"""
            rng = max(vals) - min(vals)
            n = max(decs) + 2
            return _fmt(round(round(rng, n), 10))
        ...
        if by_row:
            for r in range(r1, r2 + 1):
                ...
                results.append((r, _range_text(vals, decs)))
        else:
            for c in range(c1, c2 + 1):
                ...
                results.append((c, _range_text(vals, decs)))
