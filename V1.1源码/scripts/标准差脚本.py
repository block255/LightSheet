"""标准差脚本 — 对框选区域按行/列分组求总体标准差，输出一行或一列结果。
...
标题行/列位置不写入（保留原内容）。
标准差：总体方差平方根 √(Σ(x-μ)²/N)（除以 N，非样本方差 N-1；表格场景够用）。
...
class StdDevScript(BaseScript):
    name = '标准差脚本'
    description = '对框选区域按行/列求总体标准差，输出一行或一列'
    ...
    def run(self, sheet, params):
        ...
        def _std_text(vals: list[float], decs: list[int]) -> str:
            """总体标准差 √(Σ(x-μ)²/N) + 精度（组内最大位数 + 2，除尽不补零）。"""
            n = len(vals)
            mu = sum(vals) / n
            var = sum((v - mu) ** 2 for v in vals) / n
            std = math.sqrt(var)
            digits = max(decs) + 2
            return _fmt(round(round(std, digits), 10))
        ...
        if by_row:
            for r in range(r1, r2 + 1):
                ...
                results.append((r, _std_text(vals, decs)))
        else:
            for c in range(c1, c2 + 1):
                ...
                results.append((c, _std_text(vals, decs)))
