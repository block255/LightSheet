"""最大值脚本 — 对框选区域按行/列分组求最大值，输出一行或一列结果。
...
class MaxScript(BaseScript):
    name = '最大值脚本'
    description = '对框选区域按行/列求最大值，输出一行或一列'
    ...
    def run(self, sheet, params):
        ...
        def _max_text(vals: list[float], decs: list[int]) -> str:
            """求最大值 + 精度（组内最大位数 + 2，除尽不补零）。"""
            m = max(vals)
            n = max(decs) + 2
        ...
        if by_row:
            for r in range(r1, r2 + 1):
                ...
                results.append((r, _max_text(vals, decs)))
        else:
            for c in range(c1, c2 + 1):
                ...
                results.append((c, _max_text(vals, decs)))
