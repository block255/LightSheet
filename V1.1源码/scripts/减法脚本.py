"""减法脚本 — 第一个计算元为被减数，其余依次相减，输出一列或一行结果。
...
- 手动：全局固定 0-10 位；整数结果不补零（-3 显示 -3，1.5-0.05=1.45 保留 2 位显示 1.45）。
输出规则：标题格永远不写结果 ——
...
class SubtractScript(BaseScript):
    name = '减法脚本'
    description = '第一个计算元减其余（列/行/常数/剪贴板），输出一列或一行'
    ...
    def steps(self):
        return [
            ...
            OperandInputStep('输入计算元：每个框点右侧箭头选择 列/行/常数/剪贴板（第 1 个为被减数）',
