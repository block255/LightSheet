...
class FileHandler:
    ...
    def load(cls, file_path: str) -> SpreadsheetModel:
        """加载文件 → 新建并填充 SpreadsheetModel（多 sheet 文件取第一个）。"""
        sheets = cls.load_sheets(file_path)
        name, raw_data = sheets[0]
        model = SpreadsheetModel()
        model.load_2d(raw_data)
        model.file_path = file_path
        model.file_format = cls.detect_format(file_path)
        model.mark_clean()
        return model

    @classmethod
    def load_sheets(cls, file_path: str) -> list[tuple[str, list[list[str]]]]:
        """加载文件全部 sheet → [(sheet名, 二维列表), ...]。

        xlsx/xls 返回工作簿内全部 sheet；其他格式返回单元素 [(文件名, 数据)]。
        """
        handler, fmt = cls._get_handler(file_path)
        if fmt == 'xlsx':
            return handler.load_all(file_path)
        raw_data = handler.load(file_path)
        return [(Path(file_path).stem, raw_data)]
    ...
    @classmethod
    def save_sheets(cls, file_path: str,
                    sheets: list[tuple[str, list[list[str]]]],
                    formulas: dict = None) -> None:
        """保存全部 sheet 到文件。xlsx 整本写回；其他格式只写第一个。

        formulas（互译 P2 可选）：{sheet名: {(r0,c0): 公式文本}}，写回公式格。
        """
        handler, fmt = cls._get_handler(file_path)
        if fmt == 'xlsx':
            handler.write_all(file_path, sheets, formulas)
        else:
            handler.write(file_path, sheets[0][1])
