...
def create_menu_bar(parent) -> tuple[QMenuBar, dict[str, QAction]]:
    ...
    # 新建子菜单：三类格式（V1.1，与「导出为」同构）
    new_menu = file_menu.addMenu('新建(&N)')
    act = _add_action(new_menu, 'CSV 文件 (.csv)', QKeySequence.StandardKey.New)
    actions['new_csv'] = act
    act = _add_action(new_menu, 'Excel 文件 (.xlsx)')
    actions['new_xlsx'] = act
    act = _add_action(new_menu, '文本文件 (.txt)')
    actions['new_txt'] = act
