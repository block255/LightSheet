...
class CellEditDelegate(QStyledItemDelegate):
    ...
    # row, col, old_value — 编辑完成且值有变化（动态脚本触发）
    edit_finished = pyqtSignal(int, int, str)
    ...
    def destroyEditor(self, editor, index: QModelIndex):
        ...
        若编辑后的值相对编辑前有变化 → 发 edit_finished（动态脚本触发）。
        值未变（如点格后直接点他处、Esc 取消）不发。
        """
        old_value = getattr(editor, '_edit_info', None)
        old_value = old_value[2] if old_value else None
        self._editing_index = QModelIndex()
        super().destroyEditor(editor, index)
        if index.isValid():
            new_value = index.data(Qt.ItemDataRole.DisplayRole) or ''
            if (new_value or '') != (old_value or ''):
                self.edit_finished.emit(index.row(), index.column(),
                                        old_value or '')
...
class SpreadsheetGrid(QTableView):
    ...
    edit_committed = pyqtSignal(int, int, str)  # row, col, old_value — 值有变化的编辑
    ...
    def __init__(self, parent=None):
        ...
        # 编辑完成（delegate 关闭编辑器 = 失焦/回车提交）→ 发信号
        self._delegate.edit_finished.connect(self.edit_committed)
