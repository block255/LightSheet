...
# 共享识别/组装逻辑（与本地 Web 版同一份，见 scripts/operand_builder.py）
from scripts import operand_builder as _opb
...
class ScriptController(QObject):
    ...
    # 脚本成功运行（动态脚本模式记录用）：(脚本名, 脚本路径, params深拷贝)
    run_succeeded = pyqtSignal(str, str, dict)
    ...
    def set_model(self, model: SpreadsheetModel) -> None:
        """切换到另一个 sheet 的模型（多 sheet）。运行中切换则中止脚本。"""
        self._model = model
        if self._running:
            self.abort()
    ...
    def run_script(self, path: str) -> None:
        ...
        self._script = script
        self._script_path = path   # 记录脚本路径（动态脚本重放用）
    ...
    def _validate_numeric_reference(cells: list[str]) -> tuple[list[int], str | None]:
        """参考列/行校验（委托共享 operand_builder.validate_numeric_reference）。"""
        return _opb.validate_numeric_reference(cells)

    @staticmethod
    def _validate_find_reference(cells: list[str]) -> tuple[list[int], str | None]:
        """查找脚本参考校验（委托共享 operand_builder.validate_find_reference）。"""
        return _opb.validate_find_reference(cells)
    ...
    def _parse_text_cells(cells: list[str]) -> tuple[list[str], str | None]:
        """识别一列/一行字符串数据（委托共享 operand_builder.parse_text_cells）。"""
        return _opb.parse_text_cells(cells)

    @staticmethod
    def _parse_numeric_cells(cells: list[str]) -> tuple[str | None, int, list[float], list[int], str | None]:
        """识别一列/一行数据（委托共享 operand_builder.parse_numeric_cells，与 Web 版同源）。"""
        return _opb.parse_numeric_cells(cells)
    ...
    def _execute(self) -> None:
        ...
        try:
            ...
            if error:
                ...
                # 成功：通知动态脚本模式记录（副本，避免后续修改污染）
                import copy
                self.run_succeeded.emit(
                    self._script.name,
                    getattr(self, '_script_path', ''),
...
def _is_numeric(s: str) -> bool:
    """委托共享 operand_builder.is_numeric。"""
    return _opb.is_numeric(s)


def _count_decimals(text: str) -> int:
    """委托共享 operand_builder.count_decimals。"""
    return _opb.count_decimals(text)
